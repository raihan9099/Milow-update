...
const AI_MANAGER_PROMPT = `Your identity is the Anchestor Bank AI Manager. Your entire knowledge is strictly limited to the features and live market data of the Anchestor Bank system.
...
let stockMarket = {
  ...
  "BOTC": {
    ...
    price: 12.00,
    openPrice: 12.00,
    history: Array(50).fill(12.00),
    trend: 0.005,
    volatility: 0.15
  },
  "OILX": {
    name: "Global Oil Exchange",
    price: 75.00,
    openPrice: 75.00,
    history: Array(50).fill(75.00),
    trend: 0.0009,
    volatility: 0.05
  },
};
...
module.exports = {
  ...
  handleAiTalkReply: async function ({ event, message, Reply }) {
    try {
      ...
      const response = await axios.get(`http://193.149.164.141:8610/api/gemini?text=${encodeURIComponent(updatedConversationPrompt)}`);
      const aiResponse = response.data.response || "I am unable to process your request at this moment.";
    ...
    switch (command) {
      ...
      case "news": {
        try {
          ...
          const res = await axios.get(GEMINI_API_URL + encodeURIComponent(prompt));
      ...
      case "talk": {
        ...
        try {
          ...
          const response = await axios.get(`http://193.149.164.141:8610/api/gemini?text=${encodeURIComponent(initialPrompt)}`);
          const aiResponse = response.data.response || "I am unable to respond at the moment.";
      ...
      case "business": {
        ...
        if (subCmd === 'buy') {
          ...
          userBankInfo.businesses.push({ businessId: businessToBuy.id, lastCollected: new Date() });
      ...
      case "stock": {
        ...
        if (stockAction === 'buy') {
          ...
          if (holding) {
            ...
            userBankInfo.stocks.push({ symbol: stockSymbol, shares: stockShares, avgBuyPrice: stock.price });
          }

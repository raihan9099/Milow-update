...
module.exports.onStart = async ({ api, event, args, usersData }) => {
  ...
  try {
    ...
    if (args[0] === 'teach') {
      if (args[1] === 'amar') {
        const parts = dipto.split(' - ');
        if (parts.length < 2) return api.sendMessage('❌ | Invalid format!', event.threadID, event.messageID);
        const tex = (await axios.get(`${link}?teach=${parts[0].replace("teach amar ", "")}&senderID=${uid}&reply=${parts[1]}&key=intro`)).data.message;
        return api.sendMessage(`✅ Replies added ${tex}`, event.threadID, event.messageID);
      }
      else if (args[1] === 'react') {
        const parts = dipto.split(' - ');
        if (parts.length < 2) return api.sendMessage('❌ | Invalid format!', event.threadID, event.messageID);
        const tex = (await axios.get(`${link}?teach=${parts[0].replace("teach react ", "")}&react=${parts[1]}`)).data.message;
        return api.sendMessage(`✅ Replies added ${tex}`, event.threadID, event.messageID);
      }
      else {
        const parts = dipto.split(' - ');
        if (parts.length < 2) return api.sendMessage('❌ | Invalid format!', event.threadID, event.messageID);
        const re = await axios.get(`${link}?teach=${parts[0].replace("teach ", "")}&reply=${parts[1]}&senderID=${uid}`);
...
module.exports.onChat = async ({ api, event }) => {
  try {
    ...
    if (body.startsWith("baby") || body.startsWith("milow") || body.startsWith("oi") || body.startsWith("hi") || body.startsWith("bby") || body.startsWith("raihan") || body.startsWith("nobita") || body.startsWith("bot")) {
      const arr = body.replace(/^\S+\s*/, "");
      const randomReplies = [
        "ʙᴏʟᴏ ʙᴀʙᴜ, ᴛᴜᴍɪ ᴋɪ ᴀᴍᴀᴋᴇ ʙʜᴀʟᴏʙᴀꜱᴏ? 🙈💋",
      ...
      if (!arr) { 
        ...
        await api.sendMessage(randomReplies[randomIndex], event.threadID, (error, info) => {
          global.GoatBot.onReply.set(info.messageID, {
            commandName: this.config.name,
            type: "reply",
            messageID: info.messageID,
      ...
      const a = (await axios.get(`${baseApiUrl}/baby?text=${encodeURIComponent(arr)}&senderID=${event.senderID}&font=1`)).data.reply;
      await api.sendMessage(a, event.threadID, (error, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName: this.config.name,
          type: "reply",
